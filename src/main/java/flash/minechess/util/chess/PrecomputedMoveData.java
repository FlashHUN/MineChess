...
public class PrecomputedMoveData {
  ...
  static {
    ...
    for (int squareIndex = 0; squareIndex < 64; squareIndex++) {
      ...
      List<Byte> legalKnightJumps = new ArrayList<>();
      ...
      for (int knightJumpDelta : allKnightJumps) {
        ...
        if (knightJumpSquare >= 0 && knightJumpSquare < 64) {
          ...
          if (maxCoordMoveDst == 2) {
            ...
            knightBitboard |= (long)1 << knightJumpSquare;
          }
        }
      }
      knightMoves[squareIndex] = legalKnightJumps.toArray(new Byte[0]);
      ...
      for (int kingMoveDelta : directionOffsets) {
        ...
        if (kingMoveSquare >= 0 && kingMoveSquare < 64) {
          ...
          if (maxCoordMoveDst == 1) {
            ...
            kingAttackBitboards[squareIndex] |= (long)1 << kingMoveSquare;
          }
        }
      }
      kingMoves[squareIndex] = legalKingMoves.toArray (new Byte[0]);
      ...
      if (x > 0) {
        if (y < 7) {
          ...
          pawnAttackBitboards[squareIndex][Board.whiteIndex] |= (long)1 << (squareIndex + 7);
        }
        if (y > 0) {
          pawnCapturesBlack.add (squareIndex - 9);
          pawnAttackBitboards[squareIndex][Board.blackIndex] |= (long)1 << (squareIndex - 9);
        }
      }
      if (x < 7) {
        if (y < 7) {
          pawnCapturesWhite.add (squareIndex + 9);
          pawnAttackBitboards[squareIndex][Board.whiteIndex] |= (long)1 << (squareIndex + 9);
        }
        if (y > 0) {
          pawnCapturesBlack.add (squareIndex - 7);
          pawnAttackBitboards[squareIndex][Board.blackIndex] |= (long)1 << (squareIndex - 7);
        }
      }
      pawnAttacksWhite[squareIndex] = pawnCapturesWhite.toArray (new Integer[0]);
      pawnAttacksBlack[squareIndex] = pawnCapturesBlack.toArray (new Integer[0]);
      ...
      for (int directionIndex = 0; directionIndex < 4; directionIndex++) {
        ...
        for (int n = 0; n < numSquaresToEdge[squareIndex][directionIndex]; n++) {
          ...
          rookMoves[squareIndex] |= (long)1 << targetSquare;
        }
      }
      // Bishop moves
      for (int directionIndex = 4; directionIndex < 8; directionIndex++) {
        ...
        for (int n = 0; n < numSquaresToEdge[squareIndex][directionIndex]; n++) {
          int targetSquare = squareIndex + currentDirOffset * (n + 1);
          bishopMoves[squareIndex] |= (long)1 << targetSquare;
        }
      }
